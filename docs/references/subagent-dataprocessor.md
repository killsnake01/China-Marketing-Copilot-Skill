...
先对照 `docs/ecosystem/negative-early-warning.md` 标出负面苗头，再提取正面卖点。优先识别：

- 数据打脸：数据来源、测试条件、虚标、PPT参数
- 产品缺陷：发热、断连、漏音、续航崩、品控、翻车
- 价格背刺：首发冤种、背刺、等降价、涨价、补贴
- 信任崩塌：恰饭、软广、水军、控评、删评
- 价值观冒犯：不尊重、冒犯、低俗、价值观
...
```
负面早期预警
- {信号类型} — 等级: {低/中/高} — 依据: "{原文片段}" — 建议: {动作}
```

如果使用 `scripts/preprocess.py`，根据来源选择 `--mode`：

- 评论区导出、弹幕、用户反馈：`--mode comments`
- 评测字幕、评测稿：`--mode review`
- 用户提供的传播方案或文案：`--mode campaign`
- 知识库说明文档、模板、README：`--mode document`
